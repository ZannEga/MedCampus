...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (session.role !== 'Admin') {
    ...
    const dest = session.role === 'Doctor' ? 'login.html?role=doctor' : 'login.html';
  ...
  // ─── SHIFT CARD SELECTION (admin-schedule-add.html) ───────────────────────
  ...
  document.querySelectorAll('.status-option').forEach(card => {
    card.addEventListener('click', () => {
      document.querySelectorAll('.status-option').forEach(c => c.classList.remove('selected'));
  ...
  document.querySelectorAll('.toggle-switch input[type="checkbox"]').forEach(toggle => {
    toggle.addEventListener('change', () => {
      const label = toggle.closest('.preference-card')?.querySelector('h4')?.textContent || 'Setting';
      Toast.show(`${label} ${toggle.checked ? 'enabled' : 'disabled'}.`, 'info');
    });
  });

  // ─── MEDICINE STOCK → AUTO STATUS ─────────────────────────────────────────
  const stockInput = document.querySelector('input[type="number"]');
  const statusSel  = document.querySelector('#edit-status-display, #add-status-display');
  if (stockInput && statusSel) {
    stockInput.addEventListener('input', () => {
      const n = parseInt(stockInput.value, 10);
      statusSel.value = n <= 0 ? 'Out of Stock' : n < 20 ? 'Low Stock' : 'In Stock';
    });
  }

  // ─── USER ROLE / STATUS FILTER (admin-users.html) ─────────────────────────
  const roleFilter = document.querySelector('select[data-filter="role"], #roleFilter');
  if (roleFilter) {
    roleFilter.addEventListener('change', () => {
      const val = roleFilter.value.toLowerCase();
      document.querySelectorAll('tbody tr').forEach(row => {
        const role = row.querySelector('td:nth-child(2)')?.textContent.toLowerCase() || '';
        row.style.display = (val === 'all roles' || val === 'all' || role.includes(val)) ? '' : 'none';
      });
    });
  }
  const statusFilter = document.querySelector('select[data-filter="status"], #statusFilter');
  if (statusFilter) {
    statusFilter.addEventListener('change', () => {
      const val = statusFilter.value.toLowerCase();
      document.querySelectorAll('tbody tr').forEach(row => {
        const badge = row.querySelector('.badge')?.textContent.toLowerCase() || '';
        row.style.display = (val === 'active status' || val === 'all' || badge.includes(val)) ? '' : 'none';
      });
    });
  }

  // ─── SCHEDULE VIEW TOGGLE ─────────────────────────────────────────────────
  const weeklyBtn  = document.querySelector('[data-view="weekly"], #btnWeekly');
  ...
  if (weeklyBtn && listBtn && weekHeader) {
    weeklyBtn.addEventListener('click', () => {
      weeklyBtn.classList.add('btn-primary');    weeklyBtn.classList.remove('btn-outline');
      listBtn.classList.remove('btn-primary');   listBtn.classList.add('btn-outline');
      weekHeader.style.display = '';
    });
    listBtn.addEventListener('click', () => {
      listBtn.classList.add('btn-primary');      listBtn.classList.remove('btn-outline');
      weeklyBtn.classList.remove('btn-primary'); weeklyBtn.classList.add('btn-outline');
  ...
  document.querySelectorAll('form[data-submit]').forEach(form => {
    form.addEventListener('submit', e => {
      ...
      setTimeout(() => { if (redirect !== '#') window.location.href = redirect; }, 1000);
    });
  });
  ...
  document.querySelectorAll('[data-action="save-profile"]').forEach(btn => {
    btn.addEventListener('click', () => Toast.show('Profile updated successfully!', 'success'));
  });

  // ─── SECURITY ITEMS (admin-settings.html) ────────────────────────────────
  document.querySelectorAll('.security-item').forEach(item => {
    item.addEventListener('click', () => {
      const label = item.querySelector('span:first-child, div:first-child')
  ...
  const avatarImgs  = document.querySelectorAll('.avatar-edit-box img, #avatarPreview');
  if (avatarInput) {
    avatarInput.addEventListener('change', () => {
      const file = avatarInput.files[0];
      if (!file) return;
      const reader = new FileReader();
      reader.onload = e => avatarImgs.forEach(img => { img.src = e.target.result; });
