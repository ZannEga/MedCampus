...
(function () {
  ...
  function getAvatarSrc(session) {
    ...
    const color = session?.role === 'Doctor' ? 'a7c4a0/ffffff' : 'fca5a5/ffffff';
  ...
  function mountDropdown(isDoctor) {
    ...
    const profileHref = isDoctor ? 'doctor-profile.html' : 'patient-profile.html';
    const navProfile  = document.querySelector('.nav-profile');
    if (!navProfile) return;

    // Remove any existing dropdown wrapper
    navProfile.querySelectorAll('.profile-btn-wrap, .profile-dropdown').forEach(el => el.remove());
    ...
    trigger.addEventListener('click', (e) => {
      ...
      document.querySelectorAll('.profile-btn-wrap.open').forEach(el => el.classList.remove('open'));
      if (!isOpen) {
    ...
    document.addEventListener('click', () => {
      document.querySelectorAll('.profile-btn-wrap.open').forEach(el => {
        el.classList.remove('open');
        el.querySelector('.profile-trigger')?.setAttribute('aria-expanded', 'false');
      });
    });
    ...
    document.getElementById('dropdownLogoutBtn')?.addEventListener('click', () => {
      ...
      window.location.href = isDoctor ? 'login.html?role=doctor' : 'login.html';
    });
  }
