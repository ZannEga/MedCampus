...
const MOCK_VISITS = [
    {
        ...
        diagnosisDetail:
            "Patient presented with symptoms including a sore throat, mild nasal congestion, and a low-grade fever. Physical examination revealed mild pharyngeal erythema. Vitals are stable.",
        advice: [
        ...
        history: [
            {
                ...
                diagnosis:
                    "Mild hypertension observed during routine evaluation. Stress-related factors identified.",
                medicines: [
            ...
            },
            {
                ...
                diagnosis:
                    "Seasonal allergic rhinitis. Symptoms include nasal congestion and ocular itching.",
                medicines: [
        ...
        history: [
            {
                ...
                type: "Follow-up",
                diagnosis:
                    "Post-viral fatigue syndrome. Recovering well from previous URI.",
                medicines: [
...
const AppData = {
    ...
    setVisit(visitId) {
        ...
        if (visit)
            localStorage.setItem("mc_selected_visit", JSON.stringify(visit));
    },
    getVisit() {
        try {
            return JSON.parse(localStorage.getItem("mc_selected_visit"));
        } catch {
            return null;
        }
    },
    ...
    setQueue(data) {
        localStorage.setItem(
            "mc_active_queue",
            JSON.stringify(data || MOCK_QUEUE),
        );
    },
    getQueue() {
        try {
    ...
    getPatient() {
        try {
            return JSON.parse(localStorage.getItem("mc_current_patient"));
        } catch {
            return null;
        }
    },
    ...
    getRecord() {
        try {
            return JSON.parse(localStorage.getItem("mc_selected_record"));
        } catch {
            return null;
        }
    },
    ...
    clearBooking() {
        localStorage.removeItem("mc_booking");
    },
    clearQueue() {
        localStorage.removeItem("mc_active_queue");
    },
};
...
AppData.getSelectedUser = function () {
    try {
        return JSON.parse(localStorage.getItem("mc_admin_selected_user"));
    } catch {
        return null;
    }
};
...
const MOCK_MEDICINES = [
    ...
    },
    {
        ...
        stock: 0,
        status: "out_of_stock",
        description: "Long-acting insulin analog for diabetes management.",
        color: "red",
    },
    {
        id: "MED-004",
...
AppData.getSelectedMedicine = function () {
    try {
        return JSON.parse(localStorage.getItem("mc_admin_selected_medicine"));
    } catch {
        return null;
    }
};
...
AppData.getSelectedSchedule = function () {
    try {
        return JSON.parse(localStorage.getItem("mc_admin_selected_schedule"));
    } catch {
        return null;
    }
};
...
const MOCK_CREDENTIALS = [
    ...
    // Admin demo account (tidak ada di MOCK_USERS karena admin bukan pasien/dokter)
    {
        ...
        _inject: {
            ...
            department: "—",
            initials: "AU",
            avatar: "https://placehold.co/40x40/94a3b8/fff?text=AU",
            lastUpdated: "—",
        },
    },
];
...
AppData.login = function (email, password) {
    ...
    if (cred) {
        let user =
            cred._inject ||
            AppData.getUsers().find((u) => u.id === cred.userId);
    ...
    const storedCreds = JSON.parse(
        localStorage.getItem("mc_stored_creds") || "[]",
    );
    ...
    if (storedCred) {
        ...
        if (user.status === "suspended")
            return { ok: false, reason: "Your account has been suspended." };
        localStorage.setItem(
            "mc_session",
            JSON.stringify({
                userId: user.id,
...
AppData.getSession = function () {
    try {
        return JSON.parse(localStorage.getItem("mc_session"));
    } catch {
        return null;
    }
};
