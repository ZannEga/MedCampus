...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (progressFill) {
        ...
        let ahead = parseInt(
            (queueAheadEl?.textContent || "").match(/\d+/)?.[0] ?? 0,
        );
        ...
        const tick = setInterval(() => {
            if (progress >= 100) {
                ...
                if (queueAheadEl)
                    queueAheadEl.textContent = "0 people ahead of you";
            ...
            if (newAhead < ahead) {
                ahead = newAhead;
                if (queueAheadEl)
                    queueAheadEl.textContent = `${ahead} ${ahead === 1 ? "person" : "people"} ahead of you`;
                if (ahead > 0)
                    Toast.show(
                        `Queue updated: ${ahead} person(s) ahead of you.`,
                        "info",
                        2500,
                    );
            }
